...
from app.models import ApiLog, Message, HeartBeat, PairRequest
...
############################################################################
# Pair #####################################################################
@app.get("/pair", tags=["Pair"], status_code=201, 
    response_model=Message)
async def broker_pair(webPlatformId: str, applicationType: str, moduleId: str, request: Request):
    add_log(
        event=f"POST: New Pairing \
            {webPlatformId} and {applicationType}",
    ...
    try:
        message_value = Message(
            Source=moduleId,
            Target=webPlatformId,
            SourceApplicationType=applicationType,
            TargetApplicationType="WEB_PLATFORM",
...
async def disconnect_pairing(webPlatformId: str,moduleId:str, applicationType: str, request: Request):
    add_log(
        event=f"GET: Disconnect Pairing {webPlatformId} and {applicationType}",
        client=request.client,
    )
    try:
        message_value = Message(
        ...
        await producer.publish(
           TOPIC, "Pairing", value = str(message_value)
        )
        return message_value
    except KafkaException as ex:
        raise HTTPException(status_code=500, detail=ex.args[0].str())
...
async def ping_pairing(webPlatformId: str,moduleId:str, applicationType: str, request: Request):
    add_log(
        event=f"GET: Pinging Pairing {webPlatformId} and {applicationType}",
        client=request.client,
    )
    try:
...
async def update_pairing(webPlatformId: str,moduleId:str, applicationType: str, section: str ,request: Request):
    add_log(
        event=f"GET: Updating Pairing {webPlatformId} and {moduleId}",
        client=request.client,
    )
    try:
